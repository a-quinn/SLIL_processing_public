...
class Manager(object):
    ...
    def check_metadata(self):
        ...
        # Not currently used in OpenSim
        #for name in ('ANALOG:LABELS', 'ANALOG:DESCRIPTIONS'):
        #    if self.get(name) is None:
        #        warnings.warn('missing parameter {}'.format(name))
...
class Reader(Manager):
    ...
    # Alastair Quinn 2022
    def read_frames_nonGenerator(self):
        '''Returns only 3D point data from all frames.
        Parameters
        ----------
        None
        Returns
        -------
        frames : sequence of points
            This method generates a sequence of points, a numpy array of parsed,
            3D point data.
            The first three columns in the returned point data are the (x, y, z)
            coordinates of the observed motion capture point.
        '''
        scale = abs(self.point_scale)
        is_float = self.point_scale < 0
        ...
        pointsReturn = np.zeros(((self.last_frame() - self.first_frame() + 1), self.point_used, 3), float)
        self._handle.seek((self.header.data_block - 1) * 512)
        for frame_no in range(self.first_frame(), self.last_frame() + 1):
            n = 4 * self.header.point_count
            try: 
            ...
            pointsReturn[frame_no - 1, :, :] = raw[:, :3] * point_scale
        return pointsReturn
...
class Writer(Manager):
    ...
    def write(self, handle, labels):
        ...
        label_max_size = len(max(labels, key=len))
        ...
        #add_str('LABELS', 'labels', '', , )
        #add_str('DESCRIPTIONS', 'descriptions', ' ' )
