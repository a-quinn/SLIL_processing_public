...
#def plotGroupAvgMeansRelativeSLStats(exps):
    ...
    if True:
        ...
        rotMax = []
        for model in models:
            dataIn = cropData3(model[motionType], 2083, 4165)
        ...
        if (relative2Metacarpal):
            if (True):
                ...
                saveCache(y2new, 'statsTemp_' + motionType)
            else:
                y2new = loadCache('statsTemp_' + motionType)
        else:
            if (True):
                y2new = np.empty((3, len(kinematicAngles), l, len(xnew)), dtype=float)
                ...
                saveCache(y2new, 'statsTemp2_' + motionType)
            else:
                y2new = loadCache('statsTemp2_' + motionType)
        ...
        for angle in kinematicAngles:
            angleStats.append( getStats(angle) )
...
#        y2mergeInds[d1:d1*2] = 1
#        y2mergeInds[d1*2:]= 2
#        y2Subs = np.empty((d1*3)) # cadavers: 1 to 7
#        for i in range(len(exps)):
#            y2Subs[12*i:i*12+12] = i
#            y2Subs[d1 + 12*i:d1 + i*12+12] = i
#            y2Subs[d1*2 + 12*i:d1*2 + i*12+12] = i
#
#        indToRemove = []
#        tooManyNaNs = []
#        for ind0, i0 in enumerate(y2merge):
...
if True:
    if True:
        ...
        for indAx, ax0 in enumerate(ax):
            for clu in angleStats[indAx]['clusters']:
        ...
        for indFig, cat in enumerate(['cut', 'scaffold']): # 'normal', 
            for indAx, ax0 in enumerate(ax):
                ...
                negSD = mean - 1*np.nanstd(y2new[indFig + 1,indAx,:,:], axis=0)
                posSD = mean + 1*np.nanstd(y2new[indFig + 1,indAx,:,:], axis=0)
        ...
        for indAx, ax0 in enumerate(ax):
            ...
            if len(angleStats[indAx]['clusters']) > 0:
                ax0.text(0.40, 0.95, "p {}".format(angleStats[indAx]['p_value']),
                verticalalignment='center',
                rotation='horizontal', fontsize=14, transform=ax0.transAxes)
    ...
    #        rotation='horizontal', fontsize=18, transform=ax[3].transAxes)
        ...
        if relative2Metacarpal:
            fileName = r'\all_S&L_bone_kinematics2_mean_relativeNormal_wSD_stats_' + motionType
        else:
