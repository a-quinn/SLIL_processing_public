...
# for scaphoid relative to lunate
#%%
...
def run(motionType, makeRelativeToNormal = False,
        isMeanSTD = False,
        errorScaffold2Cut = False,
        useCache = False,
        includeStats = False,
        withinShortestX = False):
    if 'FE' in motionType:
        experiments = [
            #'11524', # C # some figures won't generate
            #'11525', # D
            '11526', # B
            #'11527', # C
            '11534', # A
        ...
        experiments = [
            #'11524', # C # some figures won't generate
            #'11525', # D # good data but short UR
            '11526', # B # only UR is bad
            #'11527', # C
            '11534', # A
            '11535', # B
            '11536', # A
            '11537', # A # only UR is bad
            '11538', # B
            '11539'  # B
            ]
    exps = experiments
...
#def plotGroupAvgMeansRelativeSLStats(exps):
    ...
    if relative2Metacarpal:
        if withinShortestX:
            ...
            # keep those within 2 std of mean
            ...
            if useCache:
                try:
                    y2new = loadCache('plot_group_3_shortestX_' + motionType)
                except:
                    print('No cache file found ({}) so generating...'.format('plot_group_3_shortestX_' + motionType))
                    cacheNotFound = True

            if (cacheNotFound or not useCache):
                ...
                saveCache(y2new, 'plot_group_3_shortestX_' + motionType)
            ...
            if useCache:
                try:
                    y2new = loadCache('plot_group_3_allX_' + motionType)
                except:
                    print('No cache file found ({}) so generating...'.format('plot_group_3_allX_' + motionType))
                    cacheNotFound = True
            ...
            if (cacheNotFound or not useCache):
                    ...
                    for model in models:
                        ...
                        for ind1, data in enumerate(dataIn): # n1, n2, c1, c2, s1, s2
                            ...
                            for ind3, c in enumerate(xnew):
                                ...
                                if len(indexGrouped) > 0:
                                    #ynew[ind3] = y[indexGrouped].mean()
                                    ynew[ind3] = np.nanmean(y[indexGrouped])
                ...
                saveCache(y2new, 'plot_group_3_allX_' + motionType)
...
##%%
    ...
    # use hand rotations arrays for scaphoid relative to lunate
    if True:
        if not makeRelativeToNormal:
            for ind2, kinematicAngle in enumerate(kinematicAngles):
                for ind1 in range(len(models)):
                    y3[0, kinematicAngles.index('hand_flexion'), ind1, :] = \
                        y3[0, kinematicAngles.index('sca_xrot'), ind1, :] - \
                        y3[0, kinematicAngles.index('lunate_flexion'), ind1, :]
                    y3[0, kinematicAngles.index('hand_deviation'), ind1, :] = \
                        y3[0, kinematicAngles.index('sca_yrot'), ind1, :] - \
                        y3[0, kinematicAngles.index('lunate_deviation'), ind1, :]
                    y3[0, kinematicAngles.index('hand_rotation'), ind1, :] = \
                        y3[0, kinematicAngles.index('sca_zrot'), ind1, :] - \
                        y3[0, kinematicAngles.index('lunate_rotation'), ind1, :]
        for ind2, kinematicAngle in enumerate(kinematicAngles):
            for ind1 in range(len(models)):
                y3[1, kinematicAngles.index('hand_flexion'), ind1, :] = \
                    y3[1, kinematicAngles.index('sca_xrot'), ind1, :] - \
                    y3[1, kinematicAngles.index('lunate_flexion'), ind1, :]
                y3[1, kinematicAngles.index('hand_deviation'), ind1, :] = \
                    y3[1, kinematicAngles.index('sca_yrot'), ind1, :] - \
                    y3[1, kinematicAngles.index('lunate_deviation'), ind1, :]
                y3[1, kinematicAngles.index('hand_rotation'), ind1, :] = \
                    y3[1, kinematicAngles.index('sca_zrot'), ind1, :] - \
                    y3[1, kinematicAngles.index('lunate_rotation'), ind1, :]
                y3[2, kinematicAngles.index('hand_flexion'), ind1, :] = \
                    y3[2, kinematicAngles.index('sca_xrot'), ind1, :] - \
                    y3[2, kinematicAngles.index('lunate_flexion'), ind1, :]
                y3[2, kinematicAngles.index('hand_deviation'), ind1, :] = \
                    y3[2, kinematicAngles.index('sca_yrot'), ind1, :] - \
                    y3[2, kinematicAngles.index('lunate_deviation'), ind1, :]
                y3[2, kinematicAngles.index('hand_rotation'), ind1, :] = \
                    y3[2, kinematicAngles.index('sca_zrot'), ind1, :] - \
                    y3[2, kinematicAngles.index('lunate_rotation'), ind1, :]
    if includeStats:
        angleStats = []
        for angle in kinematicAngles:
            #angleStats.append( getStats(angle, y2new) )
...
## %%
    ...
    #    if ii == 2:
    #        ii += 1
    #    ax.append(plt.subplot2grid((4, 3), (ii, iii)))
    ...
    rotYMin = []
    rotYMax = []
    rotXMinInd = [] # sometimes needs to be recalculated for figure formatting
    rotXMaxInd = []
    if makeRelativeToNormal:
        modelTypes = ['cut', 'scaffold']
        offest = 1
    else:
        modelTypes = ['normal', 'cut', 'scaffold']
        offest = 0

    if errorScaffold2Cut:
        modelTypes = ['cut'] # so fig colours are red

    for indFig, cat in enumerate(modelTypes): # 'normal', 
        for indAx, ax0 in enumerate(ax):
            if errorScaffold2Cut:
                yCut = y3[1,remapGraphs[indAx],:,:] # 'cut'
                yScaffold = y3[2,remapGraphs[indAx],:,:] # 'scaffold'
                y = yScaffold - yCut
            else:
                y = y3[indFig + offest,remapGraphs[indAx],:,:]
            ...
            if isMeanSTD:
                ...
                for i in range(len(y)):
                    ax0.plot(xnew, y[i], color=figColours[cat], linewidth=0.9, alpha=0.9, label=figNames[cat])

            # + - one standard deviation
            negSD = mean - 1*np.nanstd(y, axis=0)
            posSD = mean + 1*np.nanstd(y, axis=0)
            if isMeanSTD:
            ...
            if withinShortestX:
                ...
                rotYMin.append(np.nanmin(negSD))
                rotYMax.append(np.nanmax(posSD))
                for i in range(len(y)):
                    rotXMinInd.append(np.where(np.isfinite(y[i]))[0][0])
                    rotXMaxInd.append(y[i].shape[0] - 1 - np.where(np.isfinite(np.flip(y[i])))[0][0])
    minY = np.nanmin(rotYMin)
    maxY = np.nanmax(rotYMax)

    if not withinShortestX:
        minX = xnew[min(rotXMinInd)]
        maxX = xnew[max(rotXMaxInd)]
    ...
    if errorScaffold2Cut:
        figTitle += " Error"
    if makeRelativeToNormal:
        figTitle += " - Relative to " + figNames['normal']

    ax[1].text(-0.3, 1.5, figTitle,
        verticalalignment='center',
    ...
    #ax[6].text(-0.40, 0.5, 'Scaphoid relative\n      to Lunate',
    #    verticalalignment='center',
...
#        rotation='horizontal', fontsize=18, transform=ax[3].transAxes)
    ...
    fileName = r'\all_bone_kinematics'
    fileName += r'_stats'

    if errorScaffold2Cut:
        fileName += r'_errorScaffold2Cut'

    if makeRelativeToNormal:
        fileName += r'_rel2Normal'

    if relative2Metacarpal:
        fileName += r'_rel2MetAngle'
    else:
        fileName += r'_rel2RobotAngle'

    if isMeanSTD:
        fileName += r'_meanSD'
    else:
        fileName += r'_individualCAD'
    fileName += r'_' + motionType
...
for m in motions:
    ...
    run(m,
        ...
        useCache = True,
        includeStats = False,
        withinShortestX = True
        )
    run(m,
        makeRelativeToNormal = True,
        isMeanSTD = True,
        errorScaffold2Cut = False,
        useCache = True,
        includeStats = True,
